...
function buildAttacker(color = 0x79805f) {
  const group = new THREE.Group();
...
export function createHelmsDeepGame(host, callbacks = {}) {
  ...
  const laneMarkers = LANE_X.map((x) => {
  ...
  function onKeyDown(event) {
    held.add(event.key);
    if (event.key === "ArrowLeft" || event.key === "a" || event.key === "A") wallLane = Math.max(0, wallLane - 1);
    if (event.key === "ArrowRight" || event.key === "d" || event.key === "D") wallLane = Math.min(2, wallLane + 1);
  }
  ...
  return {
    ...
    enterTriviaMode() {
      triviaMode = true;
      showAttackers(ladder?.lane ?? wallLane);
    },
